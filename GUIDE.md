...
│   └── skills/       # 跨工具通用 Skill 檔案（每步驟一個 SKILL.md）
├── agent/            # MCP Server（Python，提供程式化 workflow API）
│   ├── server.py     # FastMCP 入口
│   └── tools/        # 各步驟的 context / write 工具實作
...
SpecWingman 支援多種 AI 工具，核心 workflow（`prompts/`、`specs/`、`CONSTITUTION.md`）完全共用。
...
`agent/` 目錄提供一個 Python MCP Server，讓支援 MCP 的 AI 工具以程式化方式執行整個 workflow。

每個步驟拆為兩個工具：
- **`swm_*_context`** — 組裝 prompt、CONSTITUTION 與輸入檔，回傳完整上下文給 AI 處理
- **`swm_write_*`** — 將 AI 產出寫入對應的 `specs/` 目錄

另有兩個輔助工具：
- **`swm_status`** — 掃描 `specs/` 回報各步驟完成狀態
- **`swm_next`** — 自動判斷下一步並回傳對應的完整上下文（orchestrator）
...
Claude Code 使用者可在 `.claude/settings.json` 中設定 MCP Server，即可透過 MCP 工具自動呼叫。
...
| **Claude Code (MCP)** | 自動透過 MCP 工具呼叫 `swm_*` | `.claude/settings.json` |
...
| **其他 MCP 客戶端** | 透過 MCP protocol 呼叫 `swm_*` 工具 | `agent/server.py` |
