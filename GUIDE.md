...
reqforge/
├── .claude/
│   └── commands/     # Slash commands（/rf.help、/rf.status 等）
...
| `/rf.next` | 自動判斷目前進度並執行下一步 |
| `/rf.discover` | 步驟 1：讀取輸入 → 初步探索 |
| `/rf.extract` | 步驟 2：提取正式需求 |
| `/rf.clarify` | 步驟 3：啟動需求釐清 Q&A |
| `/rf.analyze` | 步驟 4：分析需求 |
| `/rf.spec` | 步驟 5：生成需求規格書 |
| `/rf.design` | 步驟 6：生成設計文件 |
| `/rf.log` | 步驟 7：更新版本紀錄 |

> **注意：** `/rf.clarify` 用於啟動提問流程。回答問題時直接在對話中回覆即可，不需再輸入指令。
...
**指令：** `/rf.clarify`（啟動提問；回答問題時直接在對話中回覆即可）
...
2. 執行 /rf.discover（初步探索，產出 01-discovery/）
3. 執行 /rf.extract（提取正式需求，產出 02-requirements/）
4. 執行 /rf.clarify（AI 提問），直接在對話中回答問題
5. 執行 /rf.analyze → /rf.spec → /rf.design
6. 每個步驟完成後執行 /rf.log 更新版本紀錄
7. 最終在 specs/04-design-ready/ 取得完整的開發就緒文件
```

不知道從哪裡開始？執行 `/rf.status` 查看進度，或 `/rf.next` 直接跳到下一步。
...
**Q：需求確認後想修改怎麼辦？**
執行 `/rf.clarify` 重新進入釐清流程，完成後執行 `/rf.log` 記錄變更原因，保持完整的審計軌跡。
