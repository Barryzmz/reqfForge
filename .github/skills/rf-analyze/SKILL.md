...
- 若 functional-requirements.md 無 FR-### 項目，停止並提示：「請先執行 rf-extract 完成步驟 2。」
- 若 open-questions.md 有狀態為 `Open` 或 `Awaiting Answer` 的問題，警示用戶：「open-questions.md 有未回答問題，建議先執行 rf-clarify 完成釐清，再進行分析。如確認繼續，分析結果可能不完整。」
...
完成後提醒用戶：
> 步驟 4 完成。建議執行 `rf-log` 更新版本紀錄，或執行 `rf-spec` 生成需求規格書（步驟 5）。
